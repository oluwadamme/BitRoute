...
```mermaid
flowchart TD
    subgraph ApiLayer[Layer 4: API and Composition Root - BitRoute.Api]
        CTRL[Controllers: Auth, Bookings, Schedules, Webhooks]
        HUB[SignalR TelemetryHub]
        MID[ExceptionMiddleware and RateLimiter]
    end

    subgraph AppLayer[Layer 3: Application Use Cases - BitRoute.Application]
        MED[MediatR Mediator and Event Bus]
        BS[BookingService]
        AS[AuthService]
        HANDLERS[Event Handlers]
    end

    subgraph DomainLayer[Layer 1: Core Domain - BitRoute.Domain]
    ...
    subgraph InfraLayer[Layer 2: Infrastructure - BitRoute.Infrastructure]
        EF[ApplicationDbContext: EF Core Npgsql]
        UOW[UnitOfWork: Serializable Transaction and Retry Runner]
        REDIS_STORE[RedisRefreshTokenStore and RedisCacheService]
        PAYSTACK_SRV[PaystackService and Resilience Handler]
        WORKER1[ExpiredHoldSweeper HostedService]
        WORKER2[OutboxProcessor HostedService]
    end

    subgraph External[Data Stores and External Systems]
        PG[(PostgreSQL 17 Database)]
        REDIS[(Redis 7 Cache and Token Store)]
        GATEWAY[Paystack API Gateway]
    end
